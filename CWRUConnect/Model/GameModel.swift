...
@Observable
class GameModel {
    var cardDeck: [FlashCard]?
    
    func refresh() async {
        self.cardDeck = await getGameDeck()
    }
    
    func getGameDeck() async -> [FlashCard]? {
        let session = URLSession(configuration: .default)
        let userid = UserDefaults.standard.integer(forKey: "userid")
        if let url = URL(string: "\(urlString)get_game_deck/\(userid)") {
            let request = URLRequest(url: url)
            do {
                let (data, _) = try await session.data(for: request)
                let decoder = JSONDecoder()
                let deck = try decoder.decode([FlashCard].self, from: data)
                return deck
            }
            catch {
                print("Error: \(error)")
...
//    func postGameScore(user: UserChangeRequest) async -> Void {
